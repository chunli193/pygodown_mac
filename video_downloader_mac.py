...
from PyQt6.QtWidgets import (
    ...
    QGroupBox, QProgressBar, QTextEdit, QTreeWidget, QTreeWidgetItem,
    QFileDialog, QMessageBox, QButtonGroup, QFrame, QSplitter
)
...
class VideoDownloader(QMainWindow):
    ...
    def __init__(self):
        ...
        class YtLogger:
            ...
            def _filter_text(self, text):
                if not text:
                    return ""
                return ''.join(c for c in text if c.isprintable() or c in '中文中文简体繁体汉字')
    ...
    def _on_fetch_completed(self, title):
        ...
        self.status_label.setStyleSheet("color: white; background-color: #333; padding: 5px; border-radius: 3px;")
    ...
    def _on_status_updated(self, status):
        ...
        if "下载" in status or "合并" in status or "转码" in status:
            self.status_label.setStyleSheet("color: white; background-color: #2196F3; padding: 5px; border-radius: 3px;")
        else:
            self.status_label.setStyleSheet("color: white; background-color: #333; padding: 5px; border-radius: 3px;")
    ...
    def browse_ffmpeg(self):
        file, _ = QFileDialog.getOpenFileName(
            self, "选择ffmpeg", self.ffmpeg_input.text() or "/usr/local/bin",
    ...
    def fetch_video_info(self, url):
        ...
        if not YTDLP_AVAILABLE:
            QMessageBox.critical(self, "错误", "yt-dlp未安装，请运行: pip install yt-dlp")
            return
        ...
        self.fetch_thread = threading.Thread(target=self._fetch_video_info_thread, args=(url,))
    ...
    def start_download(self):
        ...
        self.download_thread = threading.Thread(
            ...
            args=(url, output_dir, video_format, quality)
        )
    ...
    def download_danmaku(self):
        ...
        try:
            thread = threading.Thread(target=self._download_danmaku_thread, args=(url, output_dir))
            thread.daemon = True
            thread.start()
            self.log("弹幕线程已启动")
        except Exception as e:
    ...
    def _download_danmaku_thread(self, url, output_dir):
        try:
            ...
            bv_id = bv_match.group(bv_match.lastindex) if bv_match.lastindex else bv_match.group(0)
    ...
    def download_video(self, url, output_dir, video_format, quality):
        try:
            ...
            QTimer.singleShot(0, lambda: self.status_label.setStyleSheet("color: white;"))
            ...
            try:
                ...
                if not self.is_downloading:
                    QTimer.singleShot(0, lambda: self.download_finished(False, "下载已取消"))
                else:
                    import traceback
                    error_detail = f"{str(e)}\n{traceback.format_exc()}"
                    QTimer.singleShot(0, lambda: self.download_finished(False, error_detail))
            finally:
                self.is_downloading = False
                self.ydl = None
        except Exception as e:
    ...
    def progress_hook(self, d):
        ...
        if status == 'downloading':
            ...
            if progress:
                ...
                QTimer.singleShot(0, lambda p=percent: self.convert_progress_bar.setValue(p))
            else:
    ...
    def download_finished(self, success, message):
        ...
        if success:
            ...
            self.status_label.setStyleSheet("color: white; background-color: #f44336; padding: 5px; border-radius: 3px;")
    ...
    def refresh_list(self):
        ...
        if hasattr(self, 'current_download_title') and self.current_download_title:
            ...
            current_item = QTreeWidgetItem([self.current_download_title, f"{percent}%", speed_text, status_text])
